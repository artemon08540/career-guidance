export default {
    ...
    async afterUpdate(event) {
      ...
      // Якщо змінився порядок, перепорядковуємо вектори
      const prevOrder = params.data.order;
      const newOrder = result.order;
  
      if (prevOrder !== undefined && newOrder !== undefined && prevOrder !== newOrder) {
        console.log(`Order changed: ${prevOrder} -> ${newOrder}`);
